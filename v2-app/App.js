...
function Page({children,scroll=true,overlay=null}){
 ...
 const swipeBackResponder=useRef(PanResponder.create({
  onMoveShouldSetPanResponder:(evt,g)=>{
   ...
   const edgeLimit=APP_SWIPE_SCREEN==='calendar'?8:16;
   const fromLeftEdge=Number(g.x0||0)<=edgeLimit;
   const horizontal=g.dx>18&&Math.abs(g.dx)>Math.abs(g.dy)*1.8;
   return fromLeftEdge&&horizontal;
  },
  onPanResponderTerminationRequest:()=>true,
  onPanResponderRelease:(evt,g)=>{
   const threshold=APP_SWIPE_SCREEN==='calendar'?150:115;
   if(g.dx>=threshold&&(g.vx>=0.25||g.dx>=threshold+35))APP_SWIPE_BACK?.();
  }
 })).current;
 ...
 return <View style={s.root} {...(Platform.OS==='web'?{}:swipeBackResponder.panHandlers)}>
  ...
  <View style={contentStyle}>{scroll?<ScrollView showsVerticalScrollIndicator={false}>{children}</ScrollView>:children}{overlay}</View>
 </View>;
}
...
function NewsVisual({uri,style}){
 const [failed,setFailed]=useState(false);
 useEffect(()=>setFailed(false),[uri]);
 if(uri&&!failed)return <Image source={{uri,cache:'force-cache'}} style={style} resizeMode="cover" onError={()=>setFailed(true)}/>;
 return <View style={[style,s.newsVisualFallback]}><TeamLogo name="SCU Torreense" style={s.newsVisualFallbackLogo}/></View>;
}
...
function ShortcutIcon({type}) {
 ...
 return <Svg width={27} height={27} viewBox="0 0 24 24">
  ...
  {type==='menu'?<><Rect x="4" y="4" width="6" height="6" rx="1.2" {...common}/><Rect x="14" y="4" width="6" height="6" rx="1.2" {...common}/><Rect x="4" y="14" width="6" height="6" rx="1.2" {...common}/><Rect x="14" y="14" width="6" height="6" rx="1.2" {...common}/></>:null}
  {type==='ticket'?<><Path d="M4 7h16v4a2 2 0 0 0 0 4v4H4v-4a2 2 0 0 0 0-4V7z" {...common}/><Path d="M12 7v12" {...common}/></>:null}
...
export default function App(){
 ...
 useEffect(()=>{let live=true;(async()=>{try{
  ...
  const [v,n,m,p,st,sc,sp,sh,bf,tv,hl]=await Promise.all([
   ...
   sb('matches?select=id,competition_id,home_team_id,away_team_id,event_type,round,starts_at,status,venue,city,home_score,away_score,attendance,summary,video_url,raw_data,competitions(name),sports(name),home:teams!matches_home_team_id_fkey(name,short_name,logo_url),away:teams!matches_away_team_id_fkey(name,short_name,logo_url)&order=starts_at.asc&limit=300'),
  ...
  const mapped=(m||[]).map(x=>{
   ...
   const ev={
    idEvent:String(x.id),
    dbId:x.id,
    _homeTeamId:x.home_team_id,
    _awayTeamId:x.away_team_id,
    _summary:x.summary||'',
    _attendance:x.attendance,
    _videoUrl:x.video_url||null,
 ...
 async function loadGameDetails(ev){
  if(!ev)return;
  const matchId=ev.dbId||ev.idEvent;
  setGameLoading(true);
  try{
   const [statsRows,lineupRows,eventRows]=await Promise.all([
    sb('match_statistics?select=id,team_id,stat_type,value,display_value&match_id=eq.'+encodeURIComponent(matchId)+'&order=id.asc'),
    sb('lineups?select=id,team_id,player_id,player_name,shirt_number,position,lineup_type,formation_position,team:teams!lineups_team_id_fkey(name,short_name),player:players!lineups_player_id_fkey(name,short_name,photo_url)&match_id=eq.'+encodeURIComponent(matchId)+'&order=team_id.asc,lineup_type.asc,formation_position.asc,id.asc'),
    sb('match_events?select=id,team_id,player_id,minute,extra_minute,event_type,description,video_url,team:teams!match_events_team_id_fkey(name,short_name),player:players!match_events_player_id_fkey(name,short_name)&match_id=eq.'+encodeURIComponent(matchId)+'&order=minute.asc,extra_minute.asc,id.asc')
   ]);
   const grouped={};
   (statsRows||[]).forEach(x=>{
    const key=String(x.stat_type||'Estatística');
    if(!grouped[key])grouped[key]={label:key.replace(/_/g,' '),home:'—',away:'—'};
    const value=x.display_value!=null&&x.display_value!==''?String(x.display_value):(x.value!=null?String(x.value):'—');
    if(String(x.team_id)===String(ev._homeTeamId))grouped[key].home=value;
    else if(String(x.team_id)===String(ev._awayTeamId))grouped[key].away=value;
   });
   setGameInfo({event:ev,stats:Object.values(grouped),lineup:lineupRows||[],timeline:eventRows||[],results:[]});
  }catch(e){
   setGameInfo({event:ev,stats:[],lineup:[],timeline:[],results:[]});
  }finally{
 ...
 async function pickMemberPhoto(source='library'){
  ...
  try{
   ...
   const result=source==='camera'
    ...
    :await ImagePicker.launchImageLibraryAsync({mediaTypes:['images'],allowsEditing:false,quality:.72,selectionLimit:1,exif:false,base64:false});
 ...
 function swipeBack(){
  ...
  const fixed={
   ...
   memberCard:'scutHub',account:'scutHub',tickets:'scutHub',notifications:'scutHub',matchDay:'scutHub',
 ...
 if(screen==='game'){
  ...
  const summaryText=ev?._summary||(ev?.strStatus==='finished'&&scoreKnown
   ?('Resultado final: '+home.name+' '+ev.intHomeScore+'–'+ev.intAwayScore+' '+away.name+'.'+(ev?.intRound?' Encontro da jornada '+ev.intRound+' da '+(ev?.strLeague||'competição')+'.':''))
  ...
  return <Page><Back title="JOGO"/>{gameLoading?<ActivityIndicator/>:<>
   ...
   {gameTab==='RESUMO'&&<View style={s.infoCard}>
    <Text style={s.body}>{summaryText}</Text>
    {ev?._attendance!=null?<Text style={[s.muted,{marginTop:7}]}>Assistência: {Number(ev._attendance).toLocaleString('pt-PT')} espectadores</Text>:null}
    {ev?._homeStanding&&ev?._awayStanding&&<Text style={[s.muted,{marginTop:7}]}>Classificação atual: {ev.strHomeTeam} {ev._homeStanding.position}.º ({ev._homeStanding.points} pts) · {ev.strAwayTeam} {ev._awayStanding.position}.º ({ev._awayStanding.points} pts)</Text>}
    {gameInfo.timeline.length>0&&<View style={s.timeline}><Text style={s.gameSectionLabel}>MOMENTOS DO JOGO</Text>{gameInfo.timeline.map((x,i)=>{
     const minute=x.minute!=null?String(x.minute)+(x.extra_minute?'+'+x.extra_minute:'')+'′':'';
     const who=x.player?.short_name||x.player?.name||'';
     const detail=x.description||String(x.event_type||'').replace(/_/g,' ');
     return <View key={x.id||i} style={s.gameEventRow}><Text style={s.gameEventMinute}>{minute}</Text><View style={s.gameEventBody}><Text style={s.gameEventTitle}>{who||detail}</Text>{who&&detail?<Text style={s.gameEventDetail}>{detail}</Text>:null}</View></View>;
    })}</View>}
   </View>}
   {gameTab==='ESTATÍSTICAS'&&<View style={s.infoCard}>{gameInfo.stats.length?<><View style={s.gameStatsHead}><Text style={s.gameStatsTeam}>{home.name}</Text><Text style={s.gameStatsLabel}>ESTATÍSTICA</Text><Text style={s.gameStatsTeam}>{away.name}</Text></View>{gameInfo.stats.map((x,i)=><View key={i} style={s.statRow}><Text style={s.gameStatValue}>{x.home}</Text><Text style={s.gameStatName}>{x.label}</Text><Text style={s.gameStatValue}>{x.away}</Text></View>)}</>:<Text style={s.muted}>Ainda não existem estatísticas oficiais carregadas para este jogo.</Text>}</View>}
   {gameTab==='ONZE'&&<View style={s.infoCard}>{gameInfo.lineup.length?lineupTeams.map(team=>{
    const starters=team.rows.filter(x=>!x.lineup_type||/titular|starter|initial/i.test(x.lineup_type));
    const subs=team.rows.filter(x=>x.lineup_type&&/suplente|sub|bench/i.test(x.lineup_type));
    const other=team.rows.filter(x=>!starters.includes(x)&&!subs.includes(x));
 ...
 if(screen==='news')return <Page><Back title="NOTÍCIAS"/>
  {newsLead?<Pressable style={s.newsLeadCard} onPress={()=>openArticle(newsLead)}>
   <NewsVisual uri={newsLead.hero} style={s.newsLeadImage}/>
   <View style={s.newsLeadBody}><Text style={s.newsEditorialMeta}>{newsLead.category}{newsLead.date?' · '+newsLead.date:''}</Text><Text style={s.newsLeadTitle} numberOfLines={3}>{cleanNewsTitle(newsLead.title)}</Text>{newsLead.excerpt?<Text style={s.newsLeadExcerpt} numberOfLines={2}>{cleanNewsText(newsLead.excerpt)}</Text>:null}</View>
  </Pressable>:null}
  {newsGrid.length?<View style={s.newsGrid}>{newsGrid.map(item=><Pressable key={item.id||item.title} style={s.newsGridCard} onPress={()=>openArticle(item)}>
   <NewsVisual uri={item.hero} style={s.newsGridImage}/>
   <View style={s.newsGridBody}><Text style={s.newsEditorialMeta} numberOfLines={1}>{item.category}{item.date?' · '+item.date:''}</Text><Text style={s.newsGridTitle} numberOfLines={3}>{cleanNewsTitle(item.title)}</Text></View>
  </Pressable>)}</View>:null}
  {newsRest.length?<View style={s.newsListSection}><Text style={s.newsListHeading}>MAIS NOTÍCIAS</Text>{newsRest.map(item=><Pressable key={item.id||item.title} style={s.newsEditorialRow} onPress={()=>openArticle(item)}>
   <NewsVisual uri={item.hero} style={s.newsEditorialRowImage}/>
   <View style={s.newsEditorialRowBody}><Text style={s.newsEditorialMeta} numberOfLines={1}>{item.category}{item.date?' · '+item.date:''}</Text><Text style={s.newsEditorialRowTitle} numberOfLines={3}>{cleanNewsTitle(item.title)}</Text></View>
  </Pressable>)}</View>:null}
 </Page>;
 ...
 if(screen==='members')return <Page><Back title="SÓCIOS"/>
  ...
  <View style={s.memberFormCard}>
   ...
   <View style={s.memberInputRow}>
    <TextInput value={memberForm.firstName} onChangeText={v=>setMemberForm({...memberForm,firstName:v})} placeholder="Nome *" placeholderTextColor="#7a8b99" style={[s.memberInput,s.memberInputHalf]}/>
    <TextInput value={memberForm.lastName} onChangeText={v=>setMemberForm({...memberForm,lastName:v})} placeholder="Apelido *" placeholderTextColor="#7a8b99" style={[s.memberInput,s.memberInputHalf]}/>
   </View>
   <TextInput value={memberForm.birthDate} onChangeText={v=>setMemberForm({...memberForm,birthDate:v})} placeholder="Data de nascimento * (AAAA-MM-DD)" placeholderTextColor="#7a8b99" style={s.memberInput}/>
   <TextInput value={memberForm.nif} onChangeText={v=>setMemberForm({...memberForm,nif:v.replace(/\D/g,'').slice(0,9)})} placeholder="NIF *" placeholderTextColor="#7a8b99" keyboardType="number-pad" style={s.memberInput}/>
   ...
   <TextInput value={memberForm.email} onChangeText={v=>setMemberForm({...memberForm,email:v})} placeholder="Email *" placeholderTextColor="#7a8b99" keyboardType="email-address" autoCapitalize="none" style={s.memberInput}/>
   <TextInput value={memberForm.phone} onChangeText={v=>setMemberForm({...memberForm,phone:v})} placeholder="Telemóvel *" placeholderTextColor="#7a8b99" keyboardType="phone-pad" style={s.memberInput}/>

   <Text style={s.memberSectionTitle}>MORADA</Text>
   <TextInput value={memberForm.address} onChangeText={v=>setMemberForm({...memberForm,address:v})} placeholder="Morada *" placeholderTextColor="#7a8b99" style={s.memberInput}/>
   <View style={s.memberInputRow}>
    <TextInput value={memberForm.postalCode} onChangeText={v=>setMemberForm({...memberForm,postalCode:v})} placeholder="Código postal *" placeholderTextColor="#7a8b99" style={[s.memberInput,s.memberInputHalf]}/>
    <TextInput value={memberForm.city} onChangeText={v=>setMemberForm({...memberForm,city:v})} placeholder="Localidade *" placeholderTextColor="#7a8b99" style={[s.memberInput,s.memberInputHalf]}/>
   </View>
   ...
   {memberNeedsGuardian?<>
    ...
    <TextInput value={memberForm.guardianName} onChangeText={v=>setMemberForm({...memberForm,guardianName:v})} placeholder="Nome do responsável *" placeholderTextColor="#7a8b99" style={s.memberInput}/>
    <TextInput value={memberForm.guardianEmail} onChangeText={v=>setMemberForm({...memberForm,guardianEmail:v})} placeholder="Email do responsável *" placeholderTextColor="#7a8b99" keyboardType="email-address" autoCapitalize="none" style={s.memberInput}/>
   </>:null}
 ...
 if(screen==='login')return <Page><Back title="LOGIN"/>
  <View style={s.loginCard}>
   <View style={s.loginIconWrap}><ShortcutIcon type="members"/></View>
   <Text style={s.loginTitle}>ÁREA DE UTILIZADOR</Text>
   <Text style={s.loginSub}>Inicia sessão para aceder à tua área pessoal.</Text>
   <TextInput value={loginForm.email} onChangeText={v=>{setLoginForm({...loginForm,email:v});setLoginMessage('')}} placeholder="Email" placeholderTextColor="#7a8b99" keyboardType="email-address" autoCapitalize="none" style={s.loginInput}/>
   <TextInput value={loginForm.password} onChangeText={v=>{setLoginForm({...loginForm,password:v});setLoginMessage('')}} placeholder="Palavra-passe" placeholderTextColor="#7a8b99" secureTextEntry style={s.loginInput}/>
   <Pressable onPress={()=>setLoginMessage('O acesso de utilizador está preparado; a autenticação será ligada à conta de sócio.')} style={s.loginButton}><Text style={s.loginButtonText}>ENTRAR</Text></Pressable>
   {loginMessage?<Text style={s.loginMessage}>{loginMessage}</Text>:null}
  </View>
 </Page>;

 function HubOption({icon,title,subtitle,onPress}){
  return <Pressable style={s.hubOption} onPress={onPress}><View style={s.hubIcon}><ShortcutIcon type={icon}/></View><Text style={s.hubOptionTitle}>{title}</Text><Text style={s.hubOptionSub} numberOfLines={2}>{subtitle}</Text></Pressable>;
 }
 function NotificationToggle({id,title,subtitle}){
  const on=!!notificationPrefs[id];
  return <Pressable style={s.notifyRow} onPress={()=>setNotificationPrefs({...notificationPrefs,[id]:!on})}><View style={s.notifyBody}><Text style={s.notifyTitle}>{title}</Text><Text style={s.notifySub}>{subtitle}</Text></View><View style={[s.notifySwitch,on&&s.notifySwitchOn]}><View style={[s.notifyKnob,on&&s.notifyKnobOn]}/></View></Pressable>;
 }

 if(screen==='scutHub')return <Page><Back title="ÁREA SCUT"/>
  <View style={s.hubIntro}><Text style={s.hubIntroTitle}>O TORREENSE CONTIGO</Text><Text style={s.hubIntroText}>Sócio, bilhetes, jogo em direto e preferências num único espaço.</Text></View>
 ...
 if(screen==='memberCard')return <Page><Back title="CARTÃO DIGITAL" to="scutHub"/>
  <View style={s.digitalCard}>
   <View style={s.digitalCardTop}><TeamLogo name="SCU Torreense" style={s.digitalCardLogo}/><View style={s.digitalCardTopText}><Text style={s.digitalCardClub}>SCU TORREENSE</Text><Text style={s.digitalCardLabel}>CARTÃO DIGITAL DE SÓCIO</Text></View></View>
   <View style={s.digitalCardLocked}><Text style={s.digitalCardLock}>▣</Text><Text style={s.digitalCardLockedTitle}>INICIA SESSÃO</Text><Text style={s.digitalCardLockedText}>Depois do login, este espaço mostrará fotografia, número de sócio, estado das quotas e QR Code.</Text></View>
   <Pressable style={s.hubPrimaryButton} onPress={()=>{setLoginMessage('');setScreen('login')}}><Text style={s.hubPrimaryButtonText}>ENTRAR NA CONTA</Text></Pressable>
  </View>
 </Page>;
 ...
 if(screen==='notifications')return <Page><Back title="NOTIFICAÇÕES" to="scutHub"/>
  <View style={s.notifyCard}>
   <Text style={s.notifyHeading}>ESCOLHE O QUE QUERES RECEBER</Text>
   <NotificationToggle id="kickoff" title="Início do jogo" subtitle="Alerta quando o jogo estiver prestes a começar."/>
   <NotificationToggle id="goals" title="Golos" subtitle="Atualizações de golo durante os jogos."/>
   <NotificationToggle id="lineup" title="Onze inicial" subtitle="Aviso quando o onze oficial estiver disponível."/>
   <NotificationToggle id="results" title="Resultado final" subtitle="Resultado assim que o jogo terminar."/>
   <NotificationToggle id="news" title="Notícias" subtitle="Principais novidades do clube."/>
   <NotificationToggle id="shop" title="Loja e promoções" subtitle="Novos produtos e campanhas selecionadas."/>
  </View>
  <Text style={s.notifyFootnote}>As preferências já funcionam nesta versão de demonstração. O envio real de push será ligado ao serviço de notificações da app.</Text>
 </Page>;

 if(screen==='matchDay')return <Page><Back title="DIA DE JOGO" to="scutHub"/>
  {matchDayEvent?<View style={s.matchDayCard}>
   <Text style={s.matchDayKicker}>{isMatchToday?'HOJE HÁ TORREENSE':'PRÓXIMO DIA DE JOGO'}</Text>
   <Text style={s.matchDayDate}>{fmtGameDate(matchDayEvent)}</Text>
   <View style={s.matchDayTeams}><View style={s.matchDayTeam}><TeamLogo name={matchDayEvent.strHomeTeam} uri={matchDayEvent.strHomeTeamBadge} style={s.matchDayLogo}/><Text style={s.matchDayTeamName}>{matchDayEvent.strHomeTeam}</Text></View><Text style={s.matchDayVs}>VS</Text><View style={s.matchDayTeam}><TeamLogo name={matchDayEvent.strAwayTeam} uri={matchDayEvent.strAwayTeamBadge} style={s.matchDayLogo}/><Text style={s.matchDayTeamName}>{matchDayEvent.strAwayTeam}</Text></View></View>
 ...
 if(screen==='benefits')return <Page><Back title="VANTAGENS"/>
  ...
  <TextInput value={benefitSearch} onChangeText={setBenefitSearch} placeholder="Pesquisar parceiro ou benefício..." placeholderTextColor="#7a8b99" style={s.benefitSearch}/>
 ...
 if(screen==='cart')return <Page overlay={<CartNotice/>}><Back title="CARRINHO" to="store"/>
  {cart.length?<>
 ...
 if(screen==='checkout')return <Page><Back title="CHECKOUT" to="cart"/>
  <View style={s.checkoutCard}>
   ...
   <TextInput value={checkoutForm.name} onChangeText={v=>setCheckoutForm({...checkoutForm,name:v})} placeholder="Nome completo" placeholderTextColor="#7a8b99" style={s.checkoutInput}/>
   <TextInput value={checkoutForm.email} onChangeText={v=>setCheckoutForm({...checkoutForm,email:v})} placeholder="Email" placeholderTextColor="#7a8b99" keyboardType="email-address" autoCapitalize="none" style={s.checkoutInput}/>
   <TextInput value={checkoutForm.phone} onChangeText={v=>setCheckoutForm({...checkoutForm,phone:v})} placeholder="Telemóvel / MB WAY" placeholderTextColor="#7a8b99" keyboardType="phone-pad" style={s.checkoutInput}/>
   <TextInput value={checkoutForm.nif} onChangeText={v=>setCheckoutForm({...checkoutForm,nif:v})} placeholder="NIF (opcional)" placeholderTextColor="#7a8b99" keyboardType="number-pad" style={s.checkoutInput}/>
   ...
   {checkoutForm.shippingMethod==='home'?<>
    <TextInput value={checkoutForm.street} onChangeText={v=>setCheckoutForm({...checkoutForm,street:v})} placeholder="Rua" placeholderTextColor="#7a8b99" style={s.checkoutInput}/>
    <View style={s.checkoutInputRow}><TextInput value={checkoutForm.number} onChangeText={v=>setCheckoutForm({...checkoutForm,number:v})} placeholder="Porta" placeholderTextColor="#7a8b99" style={[s.checkoutInput,s.checkoutInputSmall]}/><TextInput value={checkoutForm.postalCode} onChangeText={v=>setCheckoutForm({...checkoutForm,postalCode:v})} placeholder="Código postal" placeholderTextColor="#7a8b99" style={[s.checkoutInput,s.checkoutInputWide]}/></View>
    <TextInput value={checkoutForm.city} onChangeText={v=>setCheckoutForm({...checkoutForm,city:v})} placeholder="Localidade" placeholderTextColor="#7a8b99" style={s.checkoutInput}/>
   </>:null}
 ...
 if(screen==='playerDetail')return <Page>
  ...
  {selectedPlayer?<View style={s.playerDetailPage}>
   ...
   {playerDetailTab==='ESTATÍSTICAS'?<View>
    ...
    <Text style={[s.playerDetailNote,s.onDarkText]}>Estatísticas acumuladas na época 2026/27 com base nas fontes públicas sincronizadas na app.</Text>
   </View>:null}
 ...
 if(screen==='calendar')return <Page scroll={false}>
  ...
  <View style={s.calendarTabs}>
   {['CALENDÁRIO','CLASSIFICAÇÃO','PLANTEL','ESTATÍSTICAS'].map(tab=><Pressable key={tab} onPress={()=>setCalendarTab(tab)} style={[s.calendarTopTab,calendarTab===tab&&s.calendarTopTabOn]}><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.86} style={[s.calendarTopTabText,calendarTab===tab&&s.calendarTopTabTextOn]}>{tab}</Text></Pressable>)}
  </View>
   ...
   <ScrollView ref={calendarScrollRef} style={s.calendarTimeline} showsVerticalScrollIndicator={false} contentContainerStyle={s.calendarTimelineContent}>
    {calendarLoading?<ActivityIndicator/>:<>
     ...
     <View onLayout={e=>{
      ...
      </Pressable>:<View style={s.calendarBoundaryNote}><Text style={[s.muted,s.onDarkText]}>Ainda não existem resultados anteriores para esta modalidade.</Text></View>}
     </View>
...
const s=StyleSheet.create({
 ...
 memberFormCard:{padding:10,borderRadius:12,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7'},memberSectionTitle:{color:'#f1b94f',fontSize:fs(6.4),letterSpacing:.6,marginTop:5,marginBottom:6},memberInput:{height:32,borderRadius:8,borderWidth:1,borderColor:'rgba(120,164,197,.32)',backgroundColor:'#f3f6f8',color:'#17324a',fontSize:fs(6.9),paddingHorizontal:9,marginBottom:6},memberInputRow:{flexDirection:'row',justifyContent:'space-between'},memberInputHalf:{width:'49%'},memberCategoryBox:{minHeight:40,borderRadius:9,borderWidth:1,borderColor:'rgba(241,185,79,.38)',backgroundColor:'rgba(241,185,79,.08)',paddingHorizontal:9,paddingVertical:6,flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginBottom:7},memberCategoryLabel:{color:'#667b8c',fontSize:fs(5.3),letterSpacing:.4},memberCategoryName:{color:'#17324a',fontSize:fs(7.4),fontWeight:'700',marginTop:2},memberCategoryPrice:{alignItems:'flex-end'},memberCategoryMonthly:{color:'#f1b94f',fontSize:fs(7.4),fontWeight:'700'},memberCategoryAnnual:{color:'#667b8c',fontSize:fs(5.7),marginTop:1},memberGuardianNote:{color:'#667b8c',fontSize:fs(5.8),marginTop:-3,marginBottom:6},
 memberPhotoRow:{flexDirection:'row',alignItems:'center',marginBottom:8},memberPhotoFrame:{width:72,height:72,borderRadius:12,overflow:'hidden',borderWidth:1,borderColor:'rgba(241,185,79,.45)',backgroundColor:'#ffffff'},memberPhotoImage:{width:'100%',height:'100%'},memberPhotoEmpty:{flex:1,alignItems:'center',justifyContent:'center'},memberPhotoEmptyText:{color:'#667b8c',fontSize:fs(5.4),marginTop:4},memberPhotoActions:{flex:1,paddingLeft:9},memberPhotoBtn:{height:28,borderRadius:7,borderWidth:1,borderColor:'rgba(241,185,79,.48)',alignItems:'center',justifyContent:'center',marginBottom:5},memberPhotoBtnText:{color:'#f1b94f',fontSize:fs(5.9),fontWeight:'700',letterSpacing:.3},memberPhotoHint:{color:'#667b8c',fontSize:fs(5.2),lineHeight:fs(7.5)},
 memberPaymentBox:{borderRadius:10,borderWidth:1,borderColor:Platform.OS==='web'?'rgba(120,164,197,.24)':'rgba(165,205,229,.32)',backgroundColor:'#ffffff',padding:8,marginBottom:6},memberPaymentLabel:{color:'#667b8c',fontSize:fs(5.4),letterSpacing:.45,marginBottom:5},memberPaymentOptions:{flexDirection:'row',justifyContent:'space-between',marginBottom:8},memberPaymentOption:{width:'49%',height:29,borderRadius:8,borderWidth:1,borderColor:'rgba(120,164,197,.34)',alignItems:'center',justifyContent:'center'},memberPaymentMethod:{width:'49%',height:31,borderRadius:8,borderWidth:1,borderColor:'rgba(120,164,197,.34)',alignItems:'center',justifyContent:'center'},memberPaymentOptionOn:{backgroundColor:'#f1b94f',borderColor:'#f1b94f'},memberPaymentOptionText:{color:'#17324a',fontSize:fs(5.9)},memberPaymentMethodText:{color:'#17324a',fontSize:fs(6.4),fontWeight:'700'},memberPaymentOptionTextOn:{color:'#082b48',fontWeight:'800'},memberPaymentTotal:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingTop:7,borderTopWidth:1,borderTopColor:'#e5ebf0'},memberPaymentTotalLabel:{color:'#17324a',fontSize:fs(6)},memberPaymentTotalValue:{color:'#f1b94f',fontSize:fs(10),fontWeight:'800'},memberPaymentNote:{color:'#667b8c',fontSize:fs(5.2),lineHeight:fs(7.5),marginTop:5},memberFreeBox:{padding:8,borderRadius:8,backgroundColor:'rgba(241,185,79,.08)'},memberFreeText:{color:'#f1b94f',fontSize:fs(6.2),textAlign:'center'},
 memberConsent:{flexDirection:'row',alignItems:'flex-start',marginTop:8},memberCheck:{width:17,height:17,borderRadius:4,borderWidth:1,borderColor:'rgba(241,185,79,.60)',alignItems:'center',justifyContent:'center',marginRight:7,marginTop:1},memberCheckOn:{backgroundColor:'#f1b94f'},memberCheckMark:{color:'#082b48',fontSize:fs(10),fontWeight:'800'},memberConsentText:{flex:1,color:'#17324a',fontSize:fs(5.8),lineHeight:fs(9)},memberSubmit:{height:34,borderRadius:9,backgroundColor:'#f1b94f',alignItems:'center',justifyContent:'center',marginTop:10},memberSubmitText:{color:'#082b48',fontSize:fs(6.8),fontWeight:'800',letterSpacing:.45},memberMessage:{color:'#fff',fontSize:fs(6.3),lineHeight:fs(9.5),textAlign:'center',marginTop:8},memberFootnote:{color:'#e2edf3',fontSize:fs(5.4),lineHeight:fs(8),textAlign:'center',marginTop:7},
 ...
 benefitDetailCard:{padding:12,borderRadius:12,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7'},benefitDetailTop:{flexDirection:'row',alignItems:'center',marginBottom:12},benefitDetailAvatar:{width:42,height:42,borderRadius:11,backgroundColor:'rgba(241,185,79,.13)',borderWidth:1,borderColor:'rgba(241,185,79,.35)',alignItems:'center',justifyContent:'center'},benefitDetailAvatarText:{color:'#f1b94f',fontSize:fs(15),fontWeight:'800'},benefitDetailNameWrap:{flex:1,paddingHorizontal:9},benefitDetailName:{color:'#17324a',fontSize:fs(10.5),lineHeight:fs(14),fontWeight:'700'},benefitDetailCategory:{color:'#667b8c',fontSize:fs(6.2),marginTop:2},benefitDiscount:{minWidth:44,minHeight:28,paddingHorizontal:7,borderRadius:8,backgroundColor:'#f1b94f',alignItems:'center',justifyContent:'center'},benefitDiscountText:{color:'#082b48',fontSize:fs(8.1),fontWeight:'800',textAlign:'center'},benefitDetailLabel:{color:'#f1b94f',fontSize:fs(6),letterSpacing:.55,marginTop:8,marginBottom:4},benefitDetailText:{color:'#17324a',fontSize:fs(7.2),lineHeight:fs(11)},benefitAction:{height:31,borderRadius:8,borderWidth:1,borderColor:'rgba(241,185,79,.43)',marginTop:9,paddingHorizontal:10,flexDirection:'row',alignItems:'center'},benefitActionText:{color:'#f1b94f',fontSize:fs(6.4),letterSpacing:.35,flex:1},benefitActionArrow:{color:'#f1b94f',fontSize:fs(17)},
  cartNotice:{position:'absolute',left:8,right:8,bottom:8,zIndex:1000,elevation:12,minHeight:38,borderRadius:10,backgroundColor:'#eaf6ee',borderWidth:1,borderColor:'#b9dfc5',paddingHorizontal:11,paddingVertical:8,flexDirection:'row',alignItems:'center',shadowColor:'#000',shadowOpacity:.18,shadowRadius:8,shadowOffset:{width:0,height:3}},cartNoticeIcon:{color:'#23854a',fontSize:fs(9),fontWeight:'900',marginRight:7},cartNoticeText:{flex:1,color:'#195d35',fontSize:fs(6.7),fontWeight:'700',lineHeight:fs(9)},
 ...
 productDetail:{padding:10,borderRadius:12,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7'},productHero:{width:'100%',aspectRatio:1,borderRadius:10,backgroundColor:'#fff'},productName:{color:'#17324a',fontSize:fs(12),lineHeight:fs(16),marginTop:10},productPrice:{color:'#f1b94f',fontSize:fs(13),fontWeight:'700',marginTop:4},productDescription:{color:'#667b8c',fontSize:fs(7),lineHeight:fs(11),marginTop:8},optionGroup:{marginTop:11},optionTitle:{color:'#17324a',fontSize:fs(7.3),marginBottom:6},optionValues:{flexDirection:'row',flexWrap:'wrap'},optionChip:{paddingHorizontal:10,height:27,borderRadius:14,borderWidth:1,borderColor:'rgba(241,185,79,.45)',alignItems:'center',justifyContent:'center',marginRight:6,marginBottom:6},optionChipOn:{backgroundColor:'#f1b94f'},optionChipText:{color:'#f1b94f',fontSize:fs(6.5)},optionChipTextOn:{color:'#082b48',fontWeight:'700'},productAdd:{height:36,borderRadius:9,backgroundColor:'#f1b94f',alignItems:'center',justifyContent:'center',marginTop:10},productAddText:{color:'#082b48',fontSize:fs(7),fontWeight:'700',letterSpacing:.4},productOfficial:{color:'#667b8c',fontSize:fs(6.2),textAlign:'center',marginTop:9},
  checkoutCard:{padding:11,borderRadius:12,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7'},checkoutSectionTitle:{color:'#f1b94f',fontSize:fs(7.2),letterSpacing:.65,marginTop:6,marginBottom:7},checkoutInput:{height:34,borderRadius:8,borderWidth:1,borderColor:'rgba(120,164,197,.32)',backgroundColor:'#f3f6f8',color:'#17324a',fontSize:fs(7.5),paddingHorizontal:10,marginBottom:7},checkoutInputRow:{flexDirection:'row',justifyContent:'space-between'},checkoutInputSmall:{width:'31%'},checkoutInputWide:{width:'66%'},deliveryChoices:{flexDirection:'row',flexWrap:'wrap',marginBottom:8},deliveryChoice:{height:29,paddingHorizontal:10,borderRadius:15,borderWidth:1,borderColor:'rgba(241,185,79,.45)',alignItems:'center',justifyContent:'center',marginRight:6,marginBottom:5},deliveryChoiceOn:{backgroundColor:'#f1b94f'},deliveryChoiceText:{color:'#f1b94f',fontSize:fs(6.5)},deliveryChoiceTextOn:{color:'#082b48',fontWeight:'700'},mbwayChoice:{padding:9,borderRadius:9,borderWidth:1,borderColor:'rgba(241,185,79,.45)',backgroundColor:'#f3f6f8',marginBottom:9},mbwayTitle:{color:'#17324a',fontSize:fs(8.5),fontWeight:'700'},mbwayText:{color:'#667b8c',fontSize:fs(6.2),lineHeight:fs(9),marginTop:3},checkoutSummaryRow:{flexDirection:'row',justifyContent:'space-between',paddingVertical:4},checkoutSummaryLabel:{color:'#667b8c',fontSize:fs(7)},checkoutSummaryValue:{color:'#17324a',fontSize:fs(7)},checkoutSummaryTotal:{borderTopWidth:1,borderTopColor:'#dce4ea',marginTop:4,paddingTop:8},checkoutTotalLabel:{color:'#17324a',fontSize:fs(8),fontWeight:'700'},checkoutTotalValue:{color:'#f1b94f',fontSize:fs(11),fontWeight:'700'},checkoutBtnOff:{opacity:.38},checkoutMessage:{color:'#17324a',fontSize:fs(6.6),lineHeight:fs(10),textAlign:'center',marginTop:8},
  cartItem:{flexDirection:'row',padding:8,marginBottom:7,borderRadius:10,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7'},cartImage:{width:62,height:72,borderRadius:7,backgroundColor:'#fff'},cartBody:{flex:1,paddingLeft:9},cartName:{color:'#17324a',fontSize:fs(8),lineHeight:fs(11)},cartOptions:{color:'#667b8c',fontSize:fs(6.2),marginTop:2},cartPrice:{color:'#f1b94f',fontSize:fs(8),marginTop:3},qtyRow:{flexDirection:'row',alignItems:'center',marginTop:8},qtyBtn:{width:24,height:22,borderRadius:6,borderWidth:1,borderColor:'rgba(241,185,79,.55)',alignItems:'center',justifyContent:'center'},qtyText:{color:'#f1b94f',fontSize:fs(13),lineHeight:fs(16)},qtyValue:{color:'#17324a',fontSize:fs(8),minWidth:28,textAlign:'center'},cartTotalRow:{flexDirection:'row',justifyContent:'space-between',paddingVertical:10,borderTopWidth:1,borderTopColor:'#dce4ea'},cartTotalLabel:{color:'#17324a',fontSize:fs(8),letterSpacing:.6},cartTotalValue:{color:'#f1b94f',fontSize:fs(11),fontWeight:'700'},checkoutBtn:{height:36,borderRadius:9,backgroundColor:'#f1b94f',alignItems:'center',justifyContent:'center'},checkoutText:{color:'#082b48',fontSize:fs(7),fontWeight:'700',letterSpacing:.45},checkoutNote:{color:'#667b8c',fontSize:fs(5.8),lineHeight:fs(9),marginTop:7,textAlign:'center'},
 newsSection:{marginTop:12},newsHeader:{flexDirection:'row',justifyContent:'space-between',marginBottom:7},newsHeading:{color:'#fff',fontSize:fs(8.5),letterSpacing:.65},newsMore:{color:'#f1b94f',fontSize:fs(6.5)},newsCard:{minHeight:58,marginBottom:6,borderRadius:10,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7',flexDirection:'row',alignItems:'center',overflow:'hidden'},newsAccent:{width:3,alignSelf:'stretch',backgroundColor:'#a91f42'},newsThumb:{width:74,height:58,backgroundColor:'rgba(255,255,255,.06)'},newsThumbFallback:{alignItems:'center',justifyContent:'center'},newsThumbLogo:{width:34,height:40},newsBody:{flex:1,paddingHorizontal:9,paddingVertical:7},newsMeta:{color:'#f1b94f',fontSize:fs(5.8),letterSpacing:.45,marginBottom:3},newsTitle:{color:'#17324a',fontSize:fs(8),lineHeight:fs(11)},newsArrow:{color:'#17324a',fontSize:fs(18),paddingHorizontal:8},
 ...
 calendarSectionTitle:{color:'#fff',fontSize:fs(7.2),fontWeight:'800',letterSpacing:.6,marginTop:10,marginBottom:6},calendarUpcomingCard:{flexDirection:'row',borderRadius:10,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7',marginBottom:6,overflow:'hidden'},calendarUpcomingDate:{width:61,alignItems:'center',justifyContent:'center',backgroundColor:'#f3f6f8',padding:7},calendarUpcomingDay:{color:'#17324a',fontSize:fs(7),fontWeight:'800'},calendarUpcomingTime:{color:'#f1b94f',fontSize:fs(6.3),marginTop:2},calendarUpcomingBody:{flex:1,padding:7},calendarUpcomingCompetitionRow:{flexDirection:'row',alignItems:'center',marginBottom:4},calendarUpcomingCompetition:{color:'#667b8c',fontSize:fs(5.7)},calendarUpcomingTeams:{flexDirection:'row',alignItems:'center'},calendarUpcomingLogo:{width:22,height:24},calendarUpcomingTitle:{flex:1,color:'#17324a',fontSize:fs(7.3),fontWeight:'700',textAlign:'center'},calendarUpcomingVenue:{color:'#667b8c',fontSize:fs(5.3),textAlign:'center',marginTop:4},
 calendarHistoryCard:{padding:8,borderRadius:9,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7',marginBottom:5},calendarHistoryMeta:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginBottom:5},calendarHistoryMetaRight:{flexDirection:'row',alignItems:'center'},calendarHistoryDate:{color:'#f1b94f',fontSize:fs(5.6)},calendarHistoryCompetition:{color:'#667b8c',fontSize:fs(5.3)},calendarHistoryScoreRow:{flexDirection:'row',alignItems:'center'},calendarHistoryTeam:{width:'25%',color:'#17324a',fontSize:fs(6.4),fontWeight:'600',textAlign:'right'},calendarHistoryLogo:{width:21,height:23,marginHorizontal:5},calendarHistoryScore:{width:48,color:'#17324a',fontSize:fs(8.5),fontWeight:'800',textAlign:'center'},
 classificationFilterLabel:{color:'#e7f0f5',fontSize:fs(5.5),fontWeight:'700',letterSpacing:.65,marginBottom:5,marginTop:2},classificationFilterBar:{height:33,maxHeight:33,flexGrow:0,flexShrink:0,marginBottom:8},classificationCompetitionGrid:{flexDirection:'row',flexWrap:'wrap',marginBottom:8},classificationCompetitionChip:{minHeight:28,borderRadius:14,borderWidth:1,borderColor:'rgba(120,164,197,.38)',paddingHorizontal:10,paddingVertical:6,marginRight:5,marginBottom:5,justifyContent:'center'},classificationCompetitionChipText:{color:'#eef5f9',fontSize:fs(5.7),fontWeight:'700'},classificationCompetitionTitle:{color:'#fff',fontSize:fs(8.2),fontWeight:'800',letterSpacing:.25,marginBottom:7,marginTop:1},
 classificationRouteCard:{borderRadius:10,overflow:'hidden',borderWidth:1,borderColor:'#d7e0e7',backgroundColor:'#ffffff'},classificationRouteTitle:{color:'#f1b94f',fontSize:fs(6.2),fontWeight:'800',letterSpacing:.5,padding:8,borderBottomWidth:1,borderBottomColor:'#e5ebf0'},classificationRouteMatch:{padding:7,borderBottomWidth:1,borderBottomColor:'#e5ebf0'},classificationRouteMeta:{flexDirection:'row',justifyContent:'space-between',marginBottom:5},classificationRouteDate:{color:'#667b8c',fontSize:fs(5.5)},classificationRouteRound:{color:'#667b8c',fontSize:fs(5.3)},classificationRouteTeams:{flexDirection:'row',alignItems:'center'},classificationRouteLogo:{width:20,height:22},classificationRouteTeam:{flex:1,color:'#17324a',fontSize:fs(6.2),fontWeight:'600',marginHorizontal:4},classificationRouteScore:{width:42,color:'#17324a',fontSize:fs(7.7),fontWeight:'800',textAlign:'center'},
 standingsCard:{borderRadius:10,overflow:'hidden',borderWidth:1,borderColor:'#d7e0e7'},standingsHead:{height:28,flexDirection:'row',alignItems:'center',backgroundColor:'#ffffff',paddingHorizontal:6},standingsRow:{minHeight:34,flexDirection:'row',alignItems:'center',backgroundColor:'#ffffff',borderTopWidth:1,borderTopColor:'#e5ebf0',paddingHorizontal:6},standingsRowClub:{backgroundColor:'rgba(241,185,79,.11)'},standingsCell:{width:28,color:'#667b8c',fontSize:fs(6.2),textAlign:'center'},standingsPos:{width:23},standingsTeam:{flex:1,textAlign:'left'},standingsPts:{color:'#17324a',fontWeight:'800'},standingsTeamWrap:{flex:1,flexDirection:'row',alignItems:'center'},standingsLogo:{width:22,height:24,marginRight:6},standingsTeamName:{flex:1,color:'#17324a',fontSize:fs(6.6),fontWeight:'600'},
 statsHero:{flexDirection:'row',alignItems:'center',padding:11,borderRadius:11,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7',marginBottom:8},statsHeroPosition:{color:'#f1b94f',fontSize:fs(18),fontWeight:'900',marginRight:11},statsHeroTitle:{color:'#17324a',fontSize:fs(8.2),fontWeight:'800'},statsHeroSub:{color:'#667b8c',fontSize:fs(5.7),marginTop:2},statsHeroPoints:{marginLeft:'auto',color:'#17324a',fontSize:fs(9),fontWeight:'800'},statsGrid:{flexDirection:'row',flexWrap:'wrap',justifyContent:'space-between'},statsBox:{width:'32%',paddingVertical:9,alignItems:'center',borderRadius:9,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7',marginBottom:6},statsValue:{color:'#17324a',fontSize:fs(13),fontWeight:'900'},statsLabel:{color:'#667b8c',fontSize:fs(5.2),marginTop:2},statsWide:{flexDirection:'row',justifyContent:'space-around',padding:10,borderRadius:9,backgroundColor:'#ffffff',marginTop:2},statsWideLabel:{color:'#667b8c',fontSize:fs(5.4),textAlign:'center'},statsWideValue:{color:'#f1b94f',fontSize:fs(10),fontWeight:'800',textAlign:'center',marginTop:2},formRow:{flexDirection:'row'},formBadge:{width:28,height:28,borderRadius:14,alignItems:'center',justifyContent:'center',backgroundColor:'rgba(120,164,197,.30)',marginRight:6},formWin:{backgroundColor:'rgba(53,160,95,.75)'},formDraw:{backgroundColor:'rgba(204,151,38,.75)'},formLoss:{backgroundColor:'rgba(180,52,67,.75)'},formBadgeText:{color:'#fff',fontSize:fs(7),fontWeight:'800'},
  squadGroupCard:{borderRadius:10,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7',marginBottom:9,overflow:'hidden'},squadGroupTitle:{color:'#17324a',fontSize:fs(8.8),fontWeight:'800',paddingHorizontal:10,paddingVertical:9,borderBottomWidth:1,borderBottomColor:'#e5ebf0'},squadPlayerRow:{minHeight:47,flexDirection:'row',alignItems:'center',paddingHorizontal:8,borderBottomWidth:1,borderBottomColor:'#e5ebf0'},squadThumbWrap:{width:38,height:42,overflow:'hidden',borderRadius:7,backgroundColor:'rgba(255,255,255,.05)'},squadRowNumber:{width:34,color:'#f1b94f',fontSize:fs(9),fontWeight:'800',textAlign:'center'},squadRowBody:{flex:1},squadRowName:{color:'#17324a',fontSize:fs(7.4),fontWeight:'600'},squadRowArrow:{color:'#667b8c',fontSize:fs(16),marginLeft:4},
  playerDetailPage:{paddingBottom:16},playerDetailHero:{height:280,borderRadius:13,overflow:'hidden',backgroundColor:'#f4f4f4',alignItems:'center',justifyContent:'flex-end'},playerDetailPhoto:{width:'100%',height:'100%'},playerDetailHeader:{paddingTop:10},playerDetailName:{color:'#fff',fontSize:fs(12),fontWeight:'900'},playerDetailNumber:{color:'#f1b94f'},playerDetailBirth:{color:'#edf4f8',fontSize:fs(7),marginTop:3},playerDetailFacts:{flexDirection:'row',flexWrap:'wrap',justifyContent:'space-between',marginTop:12,marginBottom:6},playerDetailFact:{width:'48.5%',minHeight:60,borderRadius:9,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7',padding:8,marginBottom:6},playerDetailFactLabel:{color:'#667b8c',fontSize:fs(5.1),fontWeight:'700',letterSpacing:.35},playerDetailFactValue:{color:'#17324a',fontSize:fs(7.2),fontWeight:'700',marginTop:8},playerBirthPlaceCard:{borderRadius:9,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7',padding:8,marginBottom:12},playerBirthPlaceValue:{color:'#17324a',fontSize:fs(7.1),fontWeight:'600',marginTop:6},playerDetailTabs:{flexDirection:'row',borderBottomWidth:1,borderBottomColor:'rgba(255,255,255,.12)',marginBottom:10},playerDetailTab:{height:31,justifyContent:'center',marginRight:18,borderBottomWidth:2,borderBottomColor:'transparent'},playerDetailTabOn:{borderBottomColor:'#f1b94f'},playerDetailTabText:{color:'#dce8ef',fontSize:fs(6.4),fontWeight:'700',letterSpacing:.35},playerDetailTabTextOn:{color:'#fff'},playerDetailSectionTitle:{color:'#fff',fontSize:fs(8.3),fontWeight:'800',marginBottom:8},playerDetailStatGrid:{flexDirection:'row',flexWrap:'wrap',justifyContent:'space-between'},playerDetailStat:{width:'48.5%',minHeight:72,borderRadius:10,borderWidth:1,borderColor:'#d7e0e7',backgroundColor:'#ffffff',padding:10,marginBottom:6},playerDetailStatLabel:{color:'#17324a',fontSize:fs(6.2),fontWeight:'700'},playerDetailStatValue:{color:'#17324a',fontSize:fs(17),fontWeight:'900',marginTop:7},playerDetailInfoStrip:{flexDirection:'row',justifyContent:'space-between',marginTop:2},playerDetailInfoItem:{width:'48.5%',borderRadius:9,borderWidth:1,borderColor:'rgba(241,185,79,.24)',backgroundColor:'rgba(241,185,79,.07)',padding:9},playerDetailInfoValue:{color:'#f1b94f',fontSize:fs(7.4),fontWeight:'800',marginTop:6},playerDetailNote:{color:'#667b8c',fontSize:fs(5.8),lineHeight:fs(8.5),marginTop:8},playerDetailTextCard:{borderRadius:10,borderWidth:1,borderColor:'#d7e0e7',backgroundColor:'#ffffff',padding:11},playerDetailText:{color:'#17324a',fontSize:fs(6.7),lineHeight:fs(10)},playerDetailCareerCard:{marginTop:8,borderRadius:10,borderWidth:1,borderColor:'#d7e0e7',backgroundColor:'#ffffff',padding:10},playerDetailCareerTitle:{color:'#f1b94f',fontSize:fs(6.2),fontWeight:'800',letterSpacing:.5,marginBottom:5},playerDetailCareerRow:{flexDirection:'row',justifyContent:'space-between',paddingVertical:5,borderBottomWidth:1,borderBottomColor:'#e5ebf0'},playerDetailCareerLabel:{color:'#667b8c',fontSize:fs(6)},playerDetailCareerValue:{color:'#17324a',fontSize:fs(6.2),fontWeight:'700',textAlign:'right'},playerTrophyRow:{minHeight:38,flexDirection:'row',alignItems:'center',borderBottomWidth:1,borderBottomColor:'#e5ebf0'},playerTrophyIcon:{width:26,height:26,borderRadius:13,backgroundColor:'rgba(241,185,79,.13)',alignItems:'center',justifyContent:'center',marginRight:8},playerTrophyIconText:{color:'#f1b94f',fontSize:fs(9)},playerTrophyText:{flex:1,color:'#17324a',fontSize:fs(6.8),fontWeight:'600'},
  squadTopButton:{height:34,marginBottom:10,borderRadius:10,borderWidth:1,borderColor:'#d7e0e7',backgroundColor:'#ffffff',flexDirection:'row',alignItems:'center',paddingHorizontal:11},squadTopButtonText:{color:'#17324a',fontSize:fs(7.5),letterSpacing:.6,marginLeft:9,flex:1},squadTopArrow:{color:'#f1b94f',fontSize:fs(18)},playerGrid:{flexDirection:'row',flexWrap:'wrap',justifyContent:'space-between'},playerCard:{width:'48.5%',marginBottom:8,borderRadius:10,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7',padding:7,position:'relative'},playerPhoto:{width:'100%',aspectRatio:.86,borderRadius:8,backgroundColor:'rgba(255,255,255,.05)'},playerPhotoDetail:{width:'100%',height:'100%',backgroundColor:'#f4f4f4'},playerPhotoCompact:{width:'100%',height:'100%',borderRadius:7,backgroundColor:'rgba(255,255,255,.05)'},playerPhotoFallback:{alignItems:'center',justifyContent:'center'},playerPhotoLogo:{width:52,height:64},playerPhotoLogoCompact:{width:24,height:29},playerNumberBadge:{position:'absolute',top:12,right:12,minWidth:22,height:22,borderRadius:11,backgroundColor:'#f1b94f',alignItems:'center',justifyContent:'center'},playerNumber:{color:'#082b48',fontSize:fs(7),fontWeight:'700'},playerName:{color:'#17324a',fontSize:fs(7.4),lineHeight:fs(10),marginTop:6,fontWeight:'600'},playerPosition:{color:'#f1b94f',fontSize:fs(6.2),marginTop:2},playerMeta:{color:'#667b8c',fontSize:fs(5.8),marginTop:2},filters:{marginBottom:12},filtersContent:{paddingRight:24},filter:{height:27,paddingHorizontal:10,marginRight:6,borderRadius:14,borderWidth:1,borderColor:'#d7e0e7',justifyContent:'center',backgroundColor:'#ffffff'},filterOn:{borderColor:'#f1b94f',backgroundColor:'rgba(241,185,79,.12)'},filterText:{color:'#17324a',fontSize:fs(6.5)},filterTextOn:{color:'#f1b94f'},gameTabs:{flexDirection:'row',marginTop:10,marginBottom:8,borderRadius:10,backgroundColor:'#f3f6f8',padding:3},gameTab:{flex:1,height:29,alignItems:'center',justifyContent:'center',borderRadius:8},gameTabOn:{backgroundColor:'rgba(241,185,79,.14)',borderWidth:1,borderColor:'#f1b94f'},gameTabText:{color:'#60788a',fontSize:fs(7),letterSpacing:.3},gameTabTextOn:{color:'#f1b94f'},timeline:{marginTop:10,paddingTop:8,borderTopWidth:1,borderTopColor:'#e5ebf0'},sourceButton:{marginTop:16,height:32,borderRadius:8,borderWidth:1,borderColor:'#f1b94f',alignItems:'center',justifyContent:'center'},sourceButtonText:{color:'#f1b94f',fontSize:fs(7),letterSpacing:.4},eventCard:{flexDirection:'row',marginBottom:7,borderRadius:10,backgroundColor:'#ffffff',borderWidth:1,borderColor:'#d7e0e7',overflow:'hidden'},dateBox:{width:72,padding:9,justifyContent:'center',backgroundColor:'#f3f6f8'},dateBoxText:{color:'#17324a',fontSize:fs(8)},eventType:{color:'#f1b94f',fontSize:fs(5.8),marginTop:4},eventBody:{flex:1,padding:9},eventSport:{color:'#667b8c',fontSize:fs(6)},eventTitle:{color:'#17324a',fontSize:fs(8.5),marginVertical:3},onDarkText:{color:'#e7f0f5'}
});