...
export default function App(){
 ...
 if(screen==='calendar')return <Page scroll={false}>
  ...
  {calendarTab==='CLASSIFICAÇÃO'?<ScrollView style={s.calendarTabBody} showsVerticalScrollIndicator={false}>
   ...
   <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.classificationFilterBar} contentContainerStyle={s.filtersContent}>
    {classificationCompetitions.map(x=><Pressable key={x} onPress={()=>setClassificationCompetition(x)} style={[s.filter,activeClassificationCompetition===x&&s.filterOn]}><Text style={[s.filterText,activeClassificationCompetition===x&&s.filterTextOn]}>{x.toUpperCase()}</Text></Pressable>)}
   </ScrollView>
   ...
   </View>:<View style={s.infoCard}><Text style={s.muted}>A classificação de {activeClassificationCompetition||'esta prova'} ainda não está disponível na app.</Text></View>}
  </ScrollView>:null}
...
const s=StyleSheet.create({
 ...
 classificationFilterLabel:{color:'#8fa9bc',fontSize:5.5,fontWeight:'700',letterSpacing:.65,marginBottom:5,marginTop:2},classificationFilterBar:{height:33,maxHeight:33,flexGrow:0,flexShrink:0,marginBottom:8},classificationCompetitionTitle:{color:'#fff',fontSize:8.2,fontWeight:'800',letterSpacing:.25,marginBottom:7,marginTop:1},
