...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget _nextMatch() {
    ...
    return Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(gradient: LinearGradient(colors: [b.secondaryColor, b.primaryColor]), borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(blurRadius: 24, offset: const Offset(0, 10), color: b.primaryColor.withValues(alpha: .22))]), child: Column(children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('PRÓXIMO JOGO', style: TextStyle(color: Colors.white70, fontWeight: FontWeight.w700, letterSpacing: 1.1, fontSize: 12)), Text(when, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 12))]), const SizedBox(height: 22), Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [_team(_teamName(home), home['logoUrl'] as String?), const Text('VS', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.w800)), _team(_teamName(away), away['logoUrl'] as String?)]), if (match['venueName'] != null) ...[const SizedBox(height: 14), Text(match['venueName'] as String, style: const TextStyle(color: Colors.white70, fontSize: 12))]]));
  }
