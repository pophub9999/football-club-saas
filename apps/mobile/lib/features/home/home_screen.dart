...
class HomeScreen extends StatefulWidget {
  const HomeScreen({super.key, required this.branding});
...
class _HomeScreenState extends State<HomeScreen> {
    ...
    return CustomScrollView(
      slivers: [
        ...
        SliverPadding(
          ...
          sliver: SliverToBoxAdapter(child: _nextMatch(context)),
        ),
        SliverPadding(
          padding: const EdgeInsets.fromLTRB(20, 24, 20, 0),
          sliver: SliverToBoxAdapter(child: _sectionTitle(context, 'A tua quota')),
        ),
        SliverPadding(
        ...
        SliverPadding(
          padding: const EdgeInsets.fromLTRB(20, 24, 20, 0),
          sliver: SliverToBoxAdapter(child: _sectionTitle(context, 'Notícias')),
        ),
        SliverPadding(
        ...
        children: [
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Olá, Ricardo 👋', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
              const SizedBox(height: 4),
              Text('Sócio nº 12345', style: TextStyle(color: b.mutedTextColor, fontSize: 13)),
            ]),
          ),
          _logo(size: 46),
        ],
  ...
  Widget _logo({double size = 56}) {
    ...
    return Container(
      ...
      child: url == null
          ? Icon(Icons.shield, color: b.primaryColor, size: size * .55)
  ...
  Widget _team(String name, IconData icon) => Column(children: [
        Container(width: 58, height: 58, decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle), child: Icon(icon, color: Color(0xFF087443), size: 30)),
        const SizedBox(height: 8),
        SizedBox(width: 85, child: Text(name, textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 12))),
      ]);

  Widget _sectionTitle(BuildContext context, String title) => Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(title, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800, color: b.textColor)),
        Text('Ver todas  ›', style: TextStyle(color: b.primaryColor, fontWeight: FontWeight.w700, fontSize: 13)),
      ]);
  ...
  Widget _duesCard() => Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Quota 2026/27', style: TextStyle(color: b.textColor, fontWeight: FontWeight.w700)),
            const SizedBox(height: 5),
            Text('25,00 € · Vencida', style: TextStyle(color: b.mutedTextColor, fontSize: 13)),
          ])),
