...
class _MatchdayModeScreenState extends State<MatchdayModeScreen> {
  ...
  List<Map<String, dynamic>> _list(String key) => _config[key] is List
      ? (_config[key] as List).whereType<Map>().map((e) => Map<String, dynamic>.from(e)).toList(growable: false)
  ...
  Future<void> _load() async {
    try {
      final json = await widget.api.getJson('/clubs/current/matchday', accessToken: widget.accessToken);
      if (mounted) setState(() => _config = _map(json));
    } catch (_) {
      if (mounted) setState(() => _error = 'Não foi possível carregar as informações do dia de jogo.');
    } finally {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Dia de jogo'), backgroundColor: b.surfaceColor, foregroundColor: b.textColor),
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            _action(context, b, Icons.analytics_outlined, 'Match Centre', 'Resultado, eventos, equipas e estatísticas', () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => MatchdayScreen(branding: b, api: widget.api, accessToken: widget.accessToken, fixture: f)))),
            const SizedBox(height: 10),
            _action(context, b, Icons.confirmation_number_outlined, 'Os meus bilhetes', 'Abrir bilhete e QR de entrada', () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => TicketsScreen(branding: b, api: widget.api, accessToken: widget.accessToken)))),
            if (_loading) const Padding(padding: EdgeInsets.all(24), child: Center(child: CircularProgressIndicator())),
            if (_error != null) _info(b, Icons.error_outline, 'Informação', _error!),
            ..._entrances(b),
            ..._parking(b),
  ...
  Widget _hero(ClubBranding b, String date, String time) {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        const SizedBox(height: 16),
        Row(children: [Expanded(child: _team(f.homeName, f.homeTeam['logoUrl'] as String?)), Padding(padding: const EdgeInsets.symmetric(horizontal: 10), child: Text(score, style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900))), Expanded(child: _team(f.awayName, f.awayTeam['logoUrl'] as String?, right: true))]),
        const SizedBox(height: 16),
  ...
  List<Widget> _entrances(ClubBranding b) => _list('entrances').isEmpty
      ? [_info(b, Icons.login_outlined, 'Entradas', 'O clube ainda não configurou os acessos.')]
      : [..._list('entrances').map((e) => _info(b, Icons.login_outlined, e['name'] as String? ?? 'Entrada', e['instructions'] as String? ?? ''))];
  ...
  Widget _location(ClubBranding b, Map<String, dynamic> l) {
    final address = l['address'] as String?;
    final map = l['mapUrl'] as String?;
    final venue = widget.fixture.venueName == null ? '' : '${widget.fixture.venueName}${widget.fixture.venueCity == null ? '' : ' · ${widget.fixture.venueCity}'}';
    final lines = [if (venue.isNotEmpty) venue, if (address != null && address.isNotEmpty) address];
    return Padding(padding: const EdgeInsets.only(top: 10), child: Card(color: b.surfaceColor, margin: EdgeInsets.zero, child: ListTile(leading: Icon(Icons.directions_outlined, color: b.primaryColor), title: Text('Como chegar', style: TextStyle(color: b.textColor, fontWeight: FontWeight.w800)), subtitle: Text(lines.isEmpty ? 'Localização ainda não definida.' : lines.join('\n'), style: TextStyle(color: b.mutedTextColor)), trailing: map == null ? null : Icon(Icons.open_in_new, color: b.primaryColor))));
  }
