...
class _TeamsScreenState extends State<TeamsScreen> {
  ...
  Future<void> _load() async {
    ...
    try { final result = await repository.teams(widget.accessToken); if (mounted) setState(() => teams = result); }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: _load,
        child: loading && teams.isEmpty
            ? ListView(children: const [SizedBox(height: 260), Center(child: CircularProgressIndicator())])
            : error != null && teams.isEmpty
                ? ListView(children: [_message('Não foi possível carregar as equipas.', b)])
                : teams.isEmpty
                    ? ListView(children: [_message('Ainda não existem equipas sincronizadas.', b)])
                    : ListView.separated(
  ...
  Widget _teamCard(FootballTeam team, ClubBranding b) => Container(
        ...
        child: Row(children: [
          Container(width: 56, height: 56, decoration: BoxDecoration(color: b.backgroundColor, shape: BoxShape.circle), child: team.logoUrl == null ? Icon(Icons.shield_outlined, color: b.primaryColor, size: 30) : ClipOval(child: Image.network(team.logoUrl!, fit: BoxFit.contain, errorBuilder: (_, __, ___) => Icon(Icons.shield_outlined, color: b.primaryColor, size: 30)))),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(team.name, style: TextStyle(color: b.textColor, fontWeight: FontWeight.w800, fontSize: 16)), if (team.shortName != null && team.shortName!.isNotEmpty) Text(team.shortName!, style: TextStyle(color: b.mutedTextColor, fontSize: 13)), const SizedBox(height: 4), Text(team.provider ?? 'football', style: TextStyle(color: b.mutedTextColor, fontSize: 11))])),
        ]),
      );
