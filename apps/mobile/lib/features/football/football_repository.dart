...
class FixtureSummary {
  ...
  final String id; final DateTime kickoffAt; final String status; final Map<String, dynamic> homeTeam; final Map<String, dynamic> awayTeam; final String? venueName; final String? venueCity; final String? competitionName; final int? homeScore; final int? awayScore;
  factory FixtureSummary.fromJson(Map<String, dynamic> json) { final hs=json['homeScore']; final as=json['awayScore']; return FixtureSummary(id: json['id'] as String, kickoffAt: DateTime.parse(json['kickoffAt'] as String).toLocal(), status: json['status'] as String? ?? 'SCHEDULED', homeTeam: Map<String,dynamic>.from(json['homeTeam'] as Map), awayTeam: Map<String,dynamic>.from(json['awayTeam'] as Map), venueName: json['venueName'] as String?, venueCity: json['venueCity'] as String?, competitionName: (json['competition'] as Map?)?['name'] as String?, homeScore: hs is num ? hs.toInt() : null, awayScore: as is num ? as.toInt() : null); }
  String teamName(Map<String,dynamic> team) => (team['shortName'] as String?)?.trim().isNotEmpty == true ? team['shortName'] as String : team['name'] as String;
  String get homeName => teamName(homeTeam); String get awayName => teamName(awayTeam);
}

class FixtureDetail {
  const FixtureDetail({required this.fixture, this.events=const [], this.lineups=const [], this.stats=const []});
  final FixtureSummary fixture; final List<Map<String,dynamic>> events; final List<Map<String,dynamic>> lineups; final List<Map<String,dynamic>> stats;
...
class FootballStanding {
  ...
  final int position; final String teamName; final int points; final String? teamShortName; final String? teamLogoUrl; final int? played; final int? won; final int? drawn; final int? lost; final int? goalsFor; final int? goalsAgainst; final int? goalDifference; final String? result;
...
class FootballStandings { const FootballStandings({required this.seasonId,required this.provider,required this.rows}); final String seasonId; final String provider; final List<FootballStanding> rows; factory FootballStandings.fromJson(Map<String,dynamic> json)=>FootballStandings(seasonId:json['seasonId'] as String???'',provider:json['provider'] as String???'',rows:(json['standings'] as List???const[]).map((item)=>FootballStanding.fromJson(Map<String,dynamic>.from(item as Map))).toList(growable:false)); }
...
class FootballPlayer {
  ...
  final String externalId; final String name; final String? displayName; final String? imageUrl; final String? nationality; final String? birthDate; final int? height; final int? weight; final String? position; final String? detailedPosition; final List<Map<String,dynamic>> teams; final List<Map<String,dynamic>> statistics;
...
class FootballTeam {
  ...
  final String id; final String externalId; final String name; final String? shortName; final String? logoUrl; final String? provider;
...
class FootballRepository {
  FootballRepository(this.api); final ApiClient api;
