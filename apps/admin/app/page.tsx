...
type Section = 'dashboard' | 'branding' | 'members' | 'dues' | 'games' | 'content' | 'tickets';
...
type Fixture = { id: string; kickoffAt: string; status: string; venueName?: string | null; venueCity?: string | null; homeTeam: { name: string; shortName?: string | null }; awayTeam: { name: string; shortName?: string | null }; competition?: { name: string; season?: string | null } | null };
type Article = { id: string; slug: string; title: string; excerpt?: string | null; imageUrl?: string | null; category?: string | null; publishedAt?: string | null };
...
export default function Home() {
  ...
  async function syncGames() {
    setBusy(true);
    try { const result = await load('/admin/football/sync', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ days: 45 }) }); setFixtures(await load('/football/fixtures/upcoming?limit=50') ?? []); setError(`Sincronização concluída: ${result?.synced ?? 0} jogos recebidos (${result?.provider ?? 'provider'}).`); }
    catch (e) { setError(e instanceof Error ? e.message : 'Erro ao sincronizar jogos.'); }
  ...
  async function createArticle() {
    ...
    setBusy(true);
    try { await load('/admin/content/news', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(article) }); setArticle({ slug: '', title: '', excerpt: '', body: '', category: '', imageUrl: '', isPublished: true }); setArticles(await load('/content/news?limit=50') ?? []); }
    catch (e) { setError(e instanceof Error ? e.message : 'Erro ao criar notícia.'); }
    finally { setBusy(false); }
  }

  useEffect(() => { if (token) openSection('dashboard'); }, [token]);
  const title = ({ dashboard: 'Dashboard', branding: 'Aparência da aplicação', members: 'Sócios', dues: 'Quotas e pagamentos', games: 'Jogos', content: 'Conteúdos', tickets: 'Bilhetes' } as const)[section];
  ...
  return <div className="shell">
    <aside className="sidebar">
      ...
      <div className="nav">
        ...
        <Nav label="⭐ Rewards" disabled />
        <Nav label="🔔 Notificações" disabled />
        <Nav label="⚙️ Configurações" disabled />
      </div>
    ...
    <main className="main">
      ...
      {section === 'games' && <GamesView fixtures={fixtures} busy={busy} onSync={syncGames} />}
      {section === 'content' && <ContentView articles={articles} article={article} setArticle={setArticle} busy={busy} onCreate={createArticle} />}
...
function BrandingView({ config, setConfig }: { config: typeof defaults; setConfig: React.Dispatch<React.SetStateAction<typeof defaults>> }) { const gradient = useMemo(() => `linear-gradient(135deg, ${config.secondary}, ${config.primary})`, [config.primary, config.secondary]); const set = (key: keyof typeof defaults, value: string) => setConfig(current => ({ ...current, [key]: value })); return <div className="grid"><section className="panel"><h2>Identidade do clube</h2><div className="formGrid"><Field label="Nome do clube" value={config.name} onChange={v => set('name', v)} /><Field label="Nome curto" value={config.shortName} onChange={v => set('shortName', v)} /><Field label="Slogan" value={config.slogan} onChange={v => set('slogan', v)} full /></div><h2 className="sectionTitle">Cores</h2><div className="colors">{(['primary','secondary','accent','background','surface'] as const).map(key => <ColorField key={key} label={{ primary: 'Primária', secondary: 'Secundária', accent: 'Destaque', background: 'Fundo', surface: 'Superfície' }[key]} value={config[key]} onChange={v => set(key, v)} />)}</div><button className="save" disabled>Guardar alterações — endpoint de branding será ligado no incremento seguinte</button></section><section className="preview"><div className="phone"><div className="phoneTop"><strong>Olá, Sócio 👋</strong><div className="phoneLogo" style={{ background: config.surface, color: config.primary }}>⚽</div></div><div className="phoneContent"><div className="hero" style={{ background: gradient }}><small>PRÓXIMO JOGO</small><h3>{config.shortName}</h3><p>20 SET · 20:15 · Estádio Municipal</p></div><strong>A tua quota</strong><div className="previewCard" style={{ background: config.surface }}><strong>Quota 2026/27 · 25,00 €</strong><span>Estado da quota</span></div><strong className="messageTitle">Mensagem do clube</strong><div className="previewCard" style={{ background: config.surface }}><strong>{config.slogan}</strong><span>A tua experiência começa aqui.</span></div></div></div></div></div>; }
