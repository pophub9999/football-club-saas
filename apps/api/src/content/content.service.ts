...
export class ContentService {
  ...
  async getArticle(tenantId: string, articleIdOrSlug: string) {
    const article = await this.prisma.article.findFirst({
      where: {
        ...
        OR: [{ id: articleIdOrSlug }, { slug: articleIdOrSlug }],
      },
    });
  ...
  async createArticle(tenantId: string, userId: string, roles: string[], input: CreateArticleInput) {
    ...
    await this.prisma.auditLog.create({
      data: { tenantId, userId, action: 'article.created', resource: 'article', resourceId: article.id },
    });
    return article;
  }

  private assertEditor(roles: string[]) {
    const allowed = ['club_owner', 'club_admin', 'content_editor'];
    if (!roles.some((role) => allowed.includes(role))) throw new Error('Insufficient permissions');
  }
}
