...
export class TicketsService {
  ...
  async getMyTicket(tenantId: string, userId: string, ticketId: string) {
    const ticket = await this.prisma.ticket.findFirst({ where: { id: ticketId, tenantId, userId }, include: { event: true, scans: { orderBy: { scannedAt: 'desc' }, take: 5 } } });
    if (!ticket) throw new NotFoundException('Ticket not found');
    return ticket;
  }

  async listEvents(tenantId: string) {
    return this.prisma.ticketEvent.findMany({ where: { tenantId, startsAt: { gte: new Date() } }, orderBy: { startsAt: 'asc' }, take: 50, include: { _count: { select: { tickets: true } } } });
  }

  async listIncomingTransfers(tenantId: string, userId: string) {
    const now = new Date();
    await this.prisma.ticketTransfer.updateMany({ where: { tenantId, toUserId: userId, status: 'PENDING', expiresAt: { lte: now } }, data: { status: 'EXPIRED' } });
    return this.prisma.ticketTransfer.findMany({
      where: { tenantId, toUserId: userId, status: 'PENDING', expiresAt: { gt: now } },
      orderBy: { createdAt: 'desc' },
      include: {
        fromUser: { select: { id: true, email: true } },
        ticket: { select: { id: true, ticketNumber: true, holderName: true, section: true, row: true, seat: true, status: true, event: { select: { id: true, name: true, startsAt: true, venueName: true } } } },
      },
    });
  }

  async getQrPayload(tenantId: string, userId: string, ticketId: string) {
    const ticket = await this.prisma.ticket.findFirst({ where: { id: ticketId, tenantId, userId }, select: { id: true, tenantId: true, eventId: true, status: true, validFrom: true, validUntil: true } });
    if (!ticket) throw new NotFoundException('Ticket not found');
    if (!['ISSUED', 'ACTIVE'].includes(ticket.status)) return { ticketId: ticket.id, active: false, payload: null, expiresAt: null };
    ...
    if ((validFrom !== null && now < validFrom) || (validUntil !== null && now >= validUntil)) return { ticketId: ticket.id, active: false, payload: null, expiresAt: null };
    const expiresAt = Math.min(now + QR_TTL_SECONDS, validUntil ?? now + QR_TTL_SECONDS);
    const payload = this.signQr({ v: 1, tid: ticket.tenantId, eid: ticket.eventId, ticket: ticket.id, iat: now, exp: expiresAt });
    return { ticketId: ticket.id, active: true, payload: `fcs://ticket/v1/${payload}`, expiresAt: new Date(expiresAt * 1000).toISOString() };
  }
  ...
  verifyQrPayload(payload: string) {
    ...
    try { body = JSON.parse(Buffer.from(encodedBody, 'base64url').toString('utf8')); } catch { throw new BadRequestException('Invalid ticket QR payload'); }
    const now = Math.floor(Date.now() / 1000);
    if (body.v !== 1 || !body.tid || !body.eid || !body.ticket || !body.iat || !body.exp || body.exp <= now || body.iat > now + 10) throw new BadRequestException('Expired or invalid ticket QR payload');
    return body;
  }
  ...
  async requestTransfer(tenantId: string, fromUserId: string, ticketId: string, dto: TransferTicketDto) {
    ...
    if (dto.recipientEmail) {
      const recipient = await this.prisma.user.findFirst({ where: { email: dto.recipientEmail.trim().toLowerCase(), status: 'ACTIVE', userTenants: { some: { tenantId } } }, select: { id: true } });
      recipientUserId = recipient?.id ?? null;
    } else if (dto.recipientMemberNumber) {
      const recipient = await this.prisma.member.findFirst({ where: { tenantId, memberNumber: dto.recipientMemberNumber.trim(), userId: { not: null } }, select: { userId: true } });
  ...
  async acceptTransfer(tenantId: string, recipientUserId: string, transferId: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const updatedTicket = await tx.ticket.updateMany({ where: { id: transfer.ticketId, tenantId, userId: transfer.fromUserId, status: { in: ['ISSUED', 'ACTIVE'] } }, data: { userId: recipientUserId, status: 'ACTIVE' } });
